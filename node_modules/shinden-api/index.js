    ...
    let promise2 = new Promise(function (resolve, reject) { // Nowy Promise
        ...
        function cb(u, url) { // Funkcja Callback - Wywoływana przy wykonaniu funkcji asynchronicznej.
            ...
            info_main_element = info_main_element // Pozbycie się niepotrzebnych pustych wyników z tablicy
                ...
                .split("\n")
                .filter(value => Object
                    .keys(value).length !== 0); // Odfiltruj puste wyniki
	...
	let promise2 = new Promise(function (resolve, reject) {
        ...
        async function cb(u, url) { // Funkcja Callback - Wywoływana przy wykonaniu funkcji asynchronicznej.
            ...
            for (var i = 0; i < plr_cnt; i++) {
				...
				var prepared_title = u.match(/(?<=\stitle=").{5,15}(?=")/g)[i]+" -- "+u.match(/(?<=>)\d{2,4}p(?=<)/g)[i];
...
async function getReqi(url, before_script, after_script, first, second, third) {
    ...
    try {
		...
		await page.goto(url);
		//await page.goto("https://api4.shinden.pl/xhr/510188/player_load?auth=UGVXdToyNjM2MzAsNSwyMDIxMDQxMjEzLDMsMjcwOTcyOTk2Mw==");
        ...
        else
		{
			...
			if (first && second)await page.waitForNavigation({timeout: 60000, waitUntil: 'domcontentloaded'});
			//console.log(f)
			if(f=="wait") await page.waitForTimeout(5500)
		if(after_script) {
			//console.log("->"+third);
